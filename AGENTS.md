...
- MQTT与本地Web共存设备按Base + SDK + 应用总成本优化RAM、堆/栈峰值、CPU/主循环延迟和Flash写放大；ESP32内存较多也不能成为浪费资源的理由。优先消除冗余和峰值叠加，保留必要余量，不照搬ESP8266实现或降低保护换指标。平台协议归SDK，两个Base保持独立；只修复实际接入暴露的缺口，不以“完善基础库”扩展无关重构。
- 与本仓改动直接相关、范围可界定的必要本地检查默认执行；命令是候选入口，不是全量清单。全环境矩阵、长稳、压力、实机连接/上传/复位/OTA和物理负载先说明范围并取得确认；同一失败连续两轮修复重测仍未解决时停止并报告。
...
- `Esp8266Base` 与 `Esp32Base` 永久独立，任何时候不得合库、互相依赖或建立共同 Base。只借鉴故障证据和方案，必须按本芯片、实际资源和执行模型重新评审，不照搬对方实现或为接口一致强行重构。
- 平台专属协议、消息编码、上报和记录确认由独立平台 SDK 承担；应用/SDK 可以依赖本库，本库不依赖平台、SDK 或设备定义，也不增加特定平台模式开关。
...
- 实施前读取必要代码和文档；行为变化同步真正受影响的 README、docs、API 参考、示例及测试源码，不为局部修正改写全部文档。
- 大型改造按可独立验收的阶段任务推进，每阶段明确结果、非目标、事实、影响范围和退出条件；实施前再次复审方案，阶段结束集中报告结果、证据和剩余工作。
- 每阶段完成必要验证后，在实际修改的各责任仓库分别提交本阶段相关改动，并报告仓库和提交号；不得混入用户或其他任务的改动。没有改动不制造空提交，无法安全提交时说明阻碍。每次阶段交付必须明确下一阶段的具体任务、责任仓库、目标和完成标准，不能只列笼统的剩余方向；全部完成则明确暂无下一步。提交不等于推送或发布。
- 有证据支持且属于已确认方案的实现与修正自主完成；关键判断无把握、方案错误或需要改变目标、范围、公共契约、架构、安全与工具链支持边界时，停止相关实施并说明依据、影响和建议，取得用户决定后继续，不带疑推进、不悄悄换方案。
- 正确性、可靠复用和降低业务接入负担优先。不得为省 RAM/Flash 削弱网络重连、MQTT/TLS、停滞保护或关键存储的必要能力和余量；优先消除重复工作、冗余状态和峰值叠加，按 Base + SDK + 应用总成本评估。必需方案不能因麻烦而省略，新增维护、构建和资源代价须明确说明；不为未确认需求增加功能。
- 证书能力按实际通信场景和安全要求决定是否启用；需要时完整评估信任来源、身份与有效期校验、时间依赖、更新和失效恢复，说明资源及维护代价。不因当前省事省略必要能力，也不让无需证书的功能强制承担其依赖。
...
- `tools/test_all.sh` 保留为综合检查入口；日常与阶段交付按实际影响选择其中的定向检查和相关示例构建，不默认全量执行。
- 默认测试不烧录、不访问串口、不依赖 WiFi 或 ESP12F 在线。
- `tools/test_all.sh` 的综合范围包括静态一致性、逻辑、根项目及全部 examples 编译；这描述脚本能力，不表示日常任务必须运行它。
- `--all-envs` 仅在对应矩阵风险确有影响且获批时运行。
- 新增可自动验证的规则，优先放进 `tools/check_static.sh` 或 `tools/check_logic.py`。
- 同一模块和风险范围的相关改动合并验证，全量/多环境构建集中在确有影响的阶段末执行；无新修改、失败或证据疑点时不重复已通过的检查。纯文档修改只检查文档、链接和 diff；必要成功、失败和边界验证不省略，不用无关或重复全量测试浪费时间和 Token。
...
- 文档描述当前正确行为，不记录历史过程。
- 保留当前契约、使用说明和结果性验证证据，不长期堆积过程计划、讨论稿及阶段流水账；已被当前契约取代、无有效引用的废弃过程稿按用户授权清理，不把经验材料、未知用途文件和用户工作混作过程稿删除。
...
- 明确开展正式发布时按发布范围选择检查；开发交付不默认运行 `tools/test_all.sh` 或 `--all-envs`。
