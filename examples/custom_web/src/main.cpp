...
static const char SENSOR_PAGE[] PROGMEM =
    ...
    " <button id='sb' type='button' onclick='sendCtrl()'>Set</button>"
    " <span id='cr'></span>"
    "</form>"
    "<script>"
    ...
    "}"
    "function sendCtrl(){"
    "  var b=document.getElementById('sb');if(b.disabled)return;b.disabled=true;"
    "  var v=document.getElementById('v').value;"
    "  fetch('/api/control?value='+v,{method:'POST'})"
    "    .then(r=>r.json())"
    "    .then(d=>document.getElementById('cr').textContent=d.ok?'OK':'FAIL')"
    "    .catch(()=>document.getElementById('cr').textContent='FAIL')"
