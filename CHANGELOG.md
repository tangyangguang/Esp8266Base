...
- `FOOTER_COMPACT` 底部导航在窄屏下切换为两行左对齐面板，避免手机上 `Free heap / Up / RSSI` 状态行换行后靠右。
...
- 新增 Web Auth 内置入口，允许通过内置导航标签 API 改名；该低频入口在后续 System 导航重构中并入 System 页面。
...
- `Esp8266BaseWiFi::macAddressHex`：输出 12 位小写十六进制 STA MAC，供业务拼设备唯一 ID。
- 默认 hostname 自动追加 MAC 后 4 位十六进制（`<前缀>-<后4位>`），保证同一固件烧到多台设备时
  默认名唯一；用户手动持久化的 hostname 仍优先。
