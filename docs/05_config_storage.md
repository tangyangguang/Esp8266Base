...
MQTT broker、port、clientId、username、password 和 trust anchor 不属于库保留 key。`Esp8266BaseMQTT::configure()` 只接受运行时连接配置并复制短字段；password 非空要求 username 非空，LWT payload 非空要求 willTopic 非空。业务可自行从私有构建配置或 Config 读取，但必须自行定义业务 key、控制凭据生命周期，且不得把真实值提交到仓库。PEM trust anchor 通常超过单个 Config string 的 96B 上限，应作为业务固件的 PROGMEM 公开证书资产或其他受控来源，由长期有效的 `BearSSL::X509List` 持有；trust anchor 与 LWT payload 的持有对象必须覆盖整个 MQTT 生命周期。
...
`clearAll()` 删除所有 `/cfg_*` 文件，包括 WiFi 凭证、Web 凭证、看门狗计数和应用自定义配置。适合长按按键恢复出厂配置。仅全部删除成功后才清空deferred队列；删除失败返回false并保留待写值。最长配置key对应的主文件及`.tmp`/`.bak`恢复文件均在清理范围，文件名缓冲不截断合法路径。删除不是事务，失败前部分文件可能已删除，后续flush可能重新写回这些待写项；调用方必须处理失败，不能继续宣称恢复出厂成功。
