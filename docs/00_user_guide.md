...
除 `/health` 外，内置管理页面使用 Basic Auth。认证用户名来自 `ESP8266BASE_WEB_AUTH_USER` 或业务代码在 `Esp8266Base::begin()` 前调用的 `Esp8266BaseWeb::setDefaultAuth(user, pass)`；认证密码按编译期默认值、`setDefaultAuth()`、设备已保存的 `eb_web_pass` 顺序确定。Web 已启动后再调用 `setDefaultAuth()` 会被忽略。
