...
`tools/test_record_store.sh`新增暂存不可读写、同世代暂存重试、正式/损坏Store拒绝覆盖、条数与释放范围、CRC、标记/记录/启用失败、清理失败、容量、暂停和逆序读取检查；原恢复、切点、保护与300轮追加/释放回归通过。设备生产集成`scripts/test_history_startup.py`以真实旧源读取器、27B编码、Base Store及SDK RecordStream通过ASAN/UBSAN，覆盖高起始业务序号及原ACK映射、真实启动切换、本地查询、重启、检查点失败、4088条/127176逻辑字节、满拒绝和累计释放轮转。

Base静态/逻辑检查与`record_store`目标构建已有通过证据；真实ESP12F消费端已启用并实际调用导入和逆序API，最新构建RAM51756B/Flash603263B，原54067B门禁余2311B，设备36项回归通过。暂存导入不是另一套活跃Store；不改变正式v1格式、正常重建禁同世代或CRC/未释放保护。不把假FS、逻辑容量和编译当作物理掉电或并发运行证据；无新烧录。日志在本机`/tmp/relay-cutover-*.log`。
