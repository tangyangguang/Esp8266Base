...
ID 从1开始，每次创建新段预留一整段区间；正常重启/OTA不改变世代，物理ID允许空洞。元数据与段头通过“临时文件写入/关闭/读回 → LittleFS rename替换”提交；不先删除旧目标。轮转顺序为先持久化释放水位和预留 ID，再提交新段头覆盖已释放旧段。不能覆盖任何完整的未释放记录。段满/有不完整尾部时换段；部分释放但仍有未释放记录的段不能回收。
