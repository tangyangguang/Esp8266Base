...
if "$NM_TOOL" -C "$NO_FS_ELF" | rg 'LittleFS|Esp8266BaseConfig|Esp8266BaseFileLog|Esp8266BaseRecordStore' >/dev/null; then
...
if "$NM_TOOL" -C .pio/build/esp12f/firmware.elf | rg 'Esp8266BaseRecordStore|recordCrcStart|StoreState' >/dev/null; then
  fail "default disabled Record Store still contributes symbols"
fi
